...
// ===== UI click sound =====
const clickSoundSrc = 'assets/click.mp3';
let clickBuffer = null;

function playClickSound(){
  // clone so rapid clicks can overlap without cutting each other off
  const sfx = clickBuffer ? clickBuffer.cloneNode() : new Audio(clickSoundSrc);
  sfx.volume = 0.35;
  sfx.play().catch(() => { /* ignore: browser blocked autoplay before first interaction */ });
}
clickBuffer = new Audio(clickSoundSrc);

document.querySelectorAll(
  '.btn, .music-btn, .burger, .nav-links a, .mobile-menu a, .contact-card, .logo'
